...
public class Model {
	...
	public void creaGrafo() {
		...
		for (DefaultEdge e : archi) {
			...
			//Questo è un metodo di utilità che fa esattamente la stessa cosa
		}
		
		List<Fermata> fermateAdiacenti = Graphs.successorListOf(this.grafo, f);
		Graphs.predecessorListOf(null, null);
		//Qui essendo non orientato i successori sono uguali ai predecessori
		 */
		...
		bfv.addTraversalListener(new TraversalListener<Fermata, DefaultEdge>(){
			...
			public void edgeTraversed(EdgeTraversalEvent<DefaultEdge> e) { 
				...
				if (predecessore.containsKey(b) && !predecessore.containsKey(a)) {
					...
					// se no di sicuro conoscevo 'a' e quindi ho scoperto 'b'
			...
			public void vertexTraversed(VertexTraversalEvent<Fermata> e) {
				...
				Fermata precedente =  vertice adiacente a 'nuova' che che già sia stato raggiunto
									  (gioè è già presente nella key della mappa);
				predecessore.put(nuova, precente);*/
			}
